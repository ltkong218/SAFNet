...
model.load_state_dict(torch.load('./checkpoints/SAFNet_challenge123.pth'))

test_results_path = './img_hdr_pred_challenge123'
...
for i in range(len(dataset_eval)):
    ...
    print('Challenge123 Test {:03d}: PSNR_l={:.3f} PSNR_m={:.3f}'.format(i+1, psnr_l, psnr_m))
...
print('Challenge123 Test Average: PSNR_l={:.3f} PSNR_m={:.3f}'.format(np.array(psnr_l_list).mean(), np.array(psnr_m_list).mean()))
